(function (window, undefined) {
  ...
  window.Asc.plugin.init = function() {
    ...
    window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'positionsMessage'});
  }
  ...
  function addField(zone_type, zone_index, page_num, initx, inity, initw, inith) {
    var zoneDiv = $('<div>', {
      class: 'zone-item'
    })
  ...
  function onBtnClick(e) {
    ...
    if (id == 'cancel') {
      window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'cancelDialog'});
    } else if (id == 'confirm') {
      window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'drawPosition', data: position_list});
    } else if (id == 'allDelete') {
      window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'deletePosition', data: position_list});
    } else {
      var strs = id.split('_')
      ...
      if (fieldname == 'use') { // 应用
        ...
        window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'drawPosition', data: [{
          zone_type: zone_type * 1,
          page_num: pagenum * 1 - 1,
          v: zone_index,
          x: x * 1,
      ...
      } else if (fieldname == 'unuse') { // 删除
        console.log('删除')
        window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'deletePosition', data: [{
          zone_type: zone_type * 1,
          v: zone_index
        }]})
      }
  ...
  function init() {
    ...
    addType(ZONE_TYPE.END, '完成', parseInt(info.pages[info.pages.length-1].XLimit - 35), parseInt(info.pages[info.pages.length - 1].YLimit - 20), 20, 10)
    addType(ZONE_TYPE.PASS, '通过', parseInt(info.pages[info.pages.length-1].XLimit - 95), parseInt(info.pages[info.pages.length - 1].YLimit - 20), 20, 10)
    addType(ZONE_TYPE.STATISTICS, '统计', parseInt(info.pages[0].XLimit - 8), parseInt(info.pages[0].YLimit - 8), 2, 2)
    addType(ZONE_TYPE.CHECK, '检查', parseInt(info.pages[info.pages.length-1].XLimit - 35), 10, 20, 10)
