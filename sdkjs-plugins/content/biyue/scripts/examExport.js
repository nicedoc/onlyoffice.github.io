import { paperCanConfirm, teacherClassMyGradePrint, examImportQuestionBank, teacherExamPrintPaperSelections, examPageUpload } from './api/paper.js'
...
(function (window, undefined) {
  ...
  function confirmPaper() {
    paperCanConfirm(paper_info.paper.paper_uuid).then(res => {
      console.log('定稿成功')
    }).catch(error => {
      console.log(error)
    })
  ...
  function getGradeSelections() {
    teacherClassMyGradePrint().then(res => {
      ...
      list.forEach((item) => {
        if (item.subject_value === subjectValue) {
          if (!gradeOptions[item.grade_code]) {
            ...
            grade_content += '<input type="radio" name="grade" value="' + item.grade_code + '" /> ' + item.grade_name
          }
        }
      })
      $('#grades').html(grade_content)
      $('#grades').on('change', 'input[type="radio"][name="grade"]', function() {
    ...
    teacherExamPrintPaperSelections().then(res => {
      if (res.data) {
        ...
        for (var key in res.data) {
          print_content += '<input type="radio" name="print" value="' + res.data[key].type + '" /> ' + res.data[key].name
        }
        print_paper_type = 'k16'
        $('#printType').html(print_content)
        $(`#printType input[type="radio"][name="print"][value=${print_paper_type}]`).prop('checked', true).change();
  ...
  function onConfirm() {
    ...
    let exam_type = paper_info.page_type === 'exam_week' ? 'weekly_quiz' : 'dtk_quiz'
    ...
    examImportQuestionBank({
      ...
      window.Asc.plugin.sendToPlugin("onWindowMessage", {type: 'exportExamSuccess', data: {
  ...
  async function uploadImages() {
    ...
    for (var file of fileInput.files) {
      ...
      await examPageUpload(data).then(res => {
        console.log(res)
        pid++
      }).catch(error => {
        console.log(error)
