...
;(function (window, undefined) {
	...
	function init() {
		...
		if (headerEl) {
			...
			if (NoScoreSet) {
				...
				content += '题为'
				content +=
					'<span class="score"><input type="text" value="' +
					batchValue +
					'" onchange="checkBatchValue()" /></span>'
			...
			if (
				checkScoreTemplate() &&
				checkScoreTemplate().value &&
				layout !== 'exam_exercise'
			) {
				content +=
					'<button onclick="setScoresTemplate()">' +
					checkScoreTemplate().label +
					'分数模板</button>'
			}
		...
		if (tipEl) {
			tipEl.innerHTML = `提示：点击即可修改单题${
				layout === 'exam_exercise' ? '权重' : '分数'
			}，橙色虚线为附加题。`
		}
		...
		if (treeEl) {
			...
			if (tree) {
				for (var i = 0; i < tree.length; i++) {
					...
					treeContent +=
						'<span class="batch-set-score" style="display: inline-block;">'
					treeContent += '批量设置'
					...
					for (var j = 0; j < tree[i].question_list.length; ++j) {
						...
						tree[i].question_list[j].asks.forEach((ask) => {
							treeContent += `<span class="score"><div class="el-input"><input id=askinput${
								ask.control_id
							} value="${
								ask.v || ''
	...
	function addInputEvent(inputid, istruct, jques, kask) {
		$(inputid).on('input', function () {
			if (kask != undefined) {
				tree[istruct].question_list[jques].asks[kask].v = getScore(
					$(inputid).val()
				)
			}
			updateSum()
	...
	function onConfirm() {
			...
			if (struct.question_list) {
				struct.question_list.forEach((e) => {
					...
					e.asks.forEach((ask, index) => {
						...
						if (ask.is_ask) {
							source_data.control_list[e.control_index].ask_controls[index].v =
								ask.v
						}
					})
					scores[e.uuid] = score + ''
		...
		questionBatchEditScore({
			paper_uuid: source_data.paper_uuid,
			ques_score: scores,
		})
			.then((res) => {
				console.log('设置分数成功')
