...
function init() {
	if (!inited) {
		...
		selectElement.addEventListener('change', function () {
			...
			window.BiyueCustomData.control_list[ques_control_index].ques_type =
				selectedValue
		})
...
function addInputEvent(inputid, index) {
	$(inputid).on('input', function () {
		if (inputid == '#ques_weight') {
			window.BiyueCustomData.control_list[ques_control_index].score = getScore(
				$(inputid).val()
			)
		} else if (index != undefined) {
			window.BiyueCustomData.control_list[ques_control_index].ask_controls[
				index
			].score = getScore($(inputid).val())
		}
	})
}
	...
	if (controlData.ask_controls && controlData.ask_controls.length) {
		...
		controlData.ask_controls.forEach((ask, index) => {
			content += `<div class="item"><span class="asklabel">(${
				index + 1
			})</span><span class="score"><div class="el-input"><input id=askinput${
				ask.control_id
			} value="${
				ask.v || ''
			}"  type="text" autocomplete="off" class="el-input__inner"></div></span></div>`
		})
		$('.asks').html(content)
		controlData.ask_controls.forEach((ask, index) => {
...
export { initListener, showQuesData }
